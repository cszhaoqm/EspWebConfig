...
bool saveEepromSsid(String &strSsid, String &strPswd)
{
  ...
  memset(buf, 0, 64);
  sprintf(buf, "ssid,%s,pswd,%s\0", strSsid, strPswd);
...
void handleRootPost() {                           //Post回调函数
  ...
  connectNewWifi();                             //连接wifi
}

void initSoftAP(void) {                         //初始化AP模式
  WiFi.mode(WIFI_AP);
  WiFi.softAPConfig(apIP, apIP, IPAddress(255, 255, 255, 0));
  if (WiFi.softAP("ESPWebConfig")) {
    Serial.println();
    Serial.println(F("ESP8266 SoftAP is ready."));
  }
}
...
void initDNS(void) {                            //初始化DNS服务器
  if (dnsServer.start(DNS_PORT, "*", apIP)) {   //判断将所有地址映射到esp8266的ip上是否成功
    Serial.println(F("Start dnsserver success."));
  }
  else
    Serial.println(F("Start dnsserver failed."));
}
...
void setup() {
  Serial.begin(115200);
  WiFi.hostname("ESP8266");                     //设置ESP8266设备名
